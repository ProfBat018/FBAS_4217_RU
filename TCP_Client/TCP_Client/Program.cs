...
if (username != null)
{
    ...
    await client.ConnectAsync("127.0.0.1", 12001); // подключение к серверу по порту 12001 и адресу 
    ...
    while (true)
    {
        ...
        {
            buffer = new byte[client.ReceiveBufferSize]; // создаем буфер для чтения данных 

            await clientStream.ReadAsync(buffer, 0, client.ReceiveBufferSize); // читаем данные из потока
